    ...
    public class AccountOpeningRepository : IAccountOpeningRepository
    {
        ...
        public async Task<Models.Response> OpenAccount(AccountOpeningRequest request)
        {
            ...
            catch (TaskCanceledException tex)
            {
                ...
                _logger.LogError($"{request.FIRST_NAME} {request.LAST_NAME} : {tex.ToString()}");
            }
            catch (Exception ex)
            {
                ...
                _logger.LogError($"{request.FIRST_NAME} {request.LAST_NAME} : {ex.ToString()}");
            }
        ...
        public async Task<FreezeResponse> FreezeAccount(FreezeRequest request)
        {
            FreezeResponse res = new FreezeResponse();
            string reqString; string respMsg = string.Empty; string resultContent = string.Empty;
            int respCode = 0;
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    reqString = JsonHelper.toJson(request);
                    var content = new StringContent(reqString, Encoding.UTF8, Constant.CONTENTTYPE);
                    var result = await client.PostAsync(_settings.beURL + _settings.aeURL, content);
                    respCode = (int)result.StatusCode;
                    respMsg = result.ReasonPhrase;
                    resultContent = await result.Content.ReadAsStringAsync();
                };
            ...
            if (!string.IsNullOrEmpty(resultContent))
            {
                res = JsonHelper.fromJson<FreezeResponse>(resultContent);

                res.message = string.IsNullOrEmpty(res.status) ? Constant.ACC_CREATION_FAILED : Constant.ACC_CREATION_SUCCESSFUL;
            }
            else
            {
                res.message = Constant.UKNOWN_MSG;
